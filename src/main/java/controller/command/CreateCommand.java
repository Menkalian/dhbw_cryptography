...
public class CreateCommand implements ICommand {
    ...
    public String execute(EnterpriseNetwork network) {
        ...
        //Todo : Do I have to check if network Participant is Branch?
        //Get Participants and check for type normal
        for (int i = 0; i < network.getParticipants().size(); i++) {
            if(network.getParticipants().get(i).getName().matches(participant01) && network.getParticipants().get(i).getType().equals(Participant.ParticipantType.NORMAL)) {
                branch1 = (EnterpriseBranch) network.getParticipants().get(i);
            }else if(network.getParticipants().get(i).getName().matches(participant02) && network.getParticipants().get(i).getType().equals(Participant.ParticipantType.NORMAL))
        ...
        //ToDo : Checks die bereits im Konstruktor sind rausnehmen?
        for (Channel channel: Database.instance.getChannels()) {
            //Todo : ist das returnen in der gui?
        ...
        //Todo : welche message ^^ - "channel " + name + " from " + participant01 + " to " + participant02 + " created"
